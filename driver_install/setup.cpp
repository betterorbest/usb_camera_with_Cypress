...
int UpdateDriver()
{
	...
	*(_tcsrchr(path, _T('\\'))) = 0;
	...
	cout << "please input 'y' or 'n' to decide whether installing the driver: ";
	char c;
	cin >> c;
	if (c == 'y')
	{
		cout << "begin to install the driver, please wait...." << endl;
		flag = UpdateDriver(path, inf_name);
	}

	if (flag)
	{
		cout << "please input 'y' or 'n' to decide whether uninstall the driver: ";
		cin >> c;
		if (c == 'y')
		{

			cout << "begin to uninstall the driver, please wait...." << endl;
			UninstallDriver(inf_name);
		}
	}
