...
void MainWindow::changeReferenceParameter(unsigned short wavelen, int addOrSub)
{
	...
	qDebug() << "Wavelen: " << wavelen << " Exposure: " << m_exposureVal << " Gain: " << m_analogGainGroup[m_analogGainNum]
	...
	if (m_noAdjust)
	{
		...
		if (m_countForCapturingSpectrum == 5)
		{
			...
			temp.m_analogGain = m_analogGainNum;
		...
		if (m_countForCapturingSpectrum == 10)
		{
			...
			m_lowExposureVal = 1;
			m_highExposureVal = EXPOSURE_MAX;
			m_analogGainNum = 0;
			setAnalogGain(m_analogGainNum);
